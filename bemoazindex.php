...
Version: 1.2.2
Author: Bob Moore ,BeMoore Web Solutions Ltd
...
//Allow sessions
include ('wp-session-manager.php');
...
function azindex($atts)
{
	...
	$wp_session['called'] = 'Called'; // A string 
...
function azindex_posts_where( $where , &$wp_query ) 
{
	...
	if(isset($wp_session['called']))
	{
		if($debug)
			echo 'Called';
			
		$wp_session->reset();	//Reset the session for the next call
		...
		if(strpos($where,$id_string) === false )
		{
			if($azindex != "")
			{
				...
				if($debug)
				{
					echo " : Filter Active : Filtering $azindex ";
				}
			}			
		}
	}
